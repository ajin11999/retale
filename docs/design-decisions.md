...
- **Scope follows the tree.** A cost node's cost spreads only over the stock leaves in its own subtree, so nesting goods under a "Carton freight" node confines that charge to them (Rp40,000 over its 24 bottles), while a node wrapping everything spreads delivery-wide. A leaf nested under several cost nodes carries each one's share. A cost node with **no** goods in its subtree allocates to nothing — never inferred otherwise; the console warns before commit. See `allocateFreightByValue` in `delivery-service.ts`.
- **Multi-leg freight (delivery-wide legs).** Goods often arrive via 2–3 transits, each billed by its own expedition with its own pricing (bulked weight per PO, package count, …). A leg is a **root cost node explicitly flagged `delivery_wide`**: it spreads by value over every stock leaf of the delivery, narrowed to one purchase's leaves when `applies_to_purchase_id` is set (multi-PO deliveries where an expedition invoices per PO). Each leg carries its own courier `vendor_id`, so AP posts per expedition. Only the billed amount is recorded — no rate cards. Legs cannot contain or be nested under other items.
